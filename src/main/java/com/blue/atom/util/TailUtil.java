 ...
 * @author liguowen
 */
public class TailUtil implements Serializable {
    ...
    public TailUtil(File file, LineHandler lineHandler) {
        this(file, lineHandler, 0);
    }

    public TailUtil(File file, LineHandler lineHandler, int initReadLine) {
        this(file, CharsetUtil.CHARSET_UTF_8, lineHandler, initReadLine, DateUnit.SECOND.getMillis());
    }

    public TailUtil(File file, Charset charset, LineHandler lineHandler) {
        this(file, charset, lineHandler, 0, DateUnit.SECOND.getMillis());
    }

    public TailUtil(File file, Charset charset, LineHandler lineHandler, int initReadLine, long period) {
        checkFile(file);
    ...
    private void readTail() throws IOException {
        ...
        if (this.initReadLine > 0) {
            ...
            while (!stack.isEmpty()) {
                // TODO: 2021/12/3 调用websocket
//                System.out.println("打印:"  + stack.pop());
